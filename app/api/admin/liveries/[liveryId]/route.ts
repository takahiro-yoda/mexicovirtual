...
export async function PATCH(
  ...
  try {
    ...
    let errorMessage = 'Failed to update livery'
    ...
    if (error?.code) {
      ...
      if (error.code === 'P2025') {
        errorMessage = 'Livery not found'
        errorDetails.message = `No livery found with id: ${params.liveryId}`
      } else if (error.code === 'P2002') {
        errorMessage = 'Unique constraint violation'
        errorDetails.message = 'A livery with this name already exists for this aircraft type'
