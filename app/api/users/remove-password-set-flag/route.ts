...
export async function POST(request: NextRequest) {
  try {
    ...
    // Remove mustSetPassword claim
    const updatedClaims = { ...currentClaims }
    delete updatedClaims.mustSetPassword
    ...
    return NextResponse.json({
      success: true,
      message: 'Password set flag removed',
    })
  } catch (error: any) {
    console.error('Error removing password set flag:', error)
    return NextResponse.json(
      { error: 'Failed to remove password set flag', details: error.message },
      { status: 500 }
    )
  }
}
